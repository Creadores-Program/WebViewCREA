...
export default async function patchHtml(html, headers) {
  ...
  const strScripts = loadPolyfills()+websocketPoly+coreJsScript+urlPoly+underscore+html5ShivScript+es5shims+es6shims+dialogPoly+headpoly+dom4+webRtc+globalPoly;
  if ($('head').length > 0) {
