...
export default async function patchHtml(html, headers) {
  ...
  const strScripts = loadPolyfills()+headpoly+es5shims+es6shims+globalPoly+coreJsScript+intlPoly+urlPoly+underscore+html5ShivScript+dom4+dialogPoly+websocketPoly+webRtc;
  if ($('head').length > 0) {
